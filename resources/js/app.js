...
window.CaboSyncAlert = {
    ...
    /**
     * Alias semántico para confirmaciones de eliminación.
     * Reutiliza CaboSyncAlert.confirm con textos e iconografía de peligro.
     */
    confirmDelete: async function (titulo, texto = "", opciones = {}) {
        return this.confirm(titulo, texto, {
            icon: "warning",
...
/**
 * Solicitudes que NO deben mostrar el preloader global
 * (peticiones frecuentes o de UX ligera, ej. autoguardado de horas extra).
 *
 * Uso desde el frontend:
 *   axios.post(url, data, { skipPreloader: true })
 */
axios.interceptors.request.use(
    (config) => {
        // Si la petición pide saltar el preloader, no lo mostramos
        if (config.skipPreloader === true) {
            config.__skipPreloader = true;
            return config;
        }
...
axios.interceptors.response.use(
    (response) => {
        // Si la request saltó el preloader, no decrementamos
        if (response.config?.__skipPreloader) {
            return response;
        }
    ...
    (error) => {
        if (!error.config?.__skipPreloader) {
            peticionesActivas = Math.max(0, peticionesActivas - 1);
            if (peticionesActivas === 0) CaboSyncLoader.hide();
        }
        ...
        if (error.response) {
            ...
            if (status === 419) {
                ...
                CaboSyncAlert.error(
                    "Error interno del servidor. Contacta a soporte.",
                );
            } else if (status !== 401) {
                CaboSyncAlert.error(
